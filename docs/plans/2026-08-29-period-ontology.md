...
- This plan's own tasks (1-10 below) stop at the data layer and do not touch `web/`/`server/`, and do not build the Holocene/geological display layer (per `ONTOLOGY.md`, that's a static UI asset for the future timeline plan, not a `Period`-tree citizen). One exception, already done, outside the numbered tasks: `Polity.region`/`culture_group` were removed entirely on 2026-08-29 (schema, all 4,671 YAML files, and their few live references in `pipeline/apply_review_decisions.py`, `pipeline/wd_to_yaml.py`, `server/app.py`, and `web/app.js`'s detail drawer/grouping heuristic) — see `ONTOLOGY.md`'s "What replaced region/culture_group" section. Not a numbered task here since it was a direct removal, not something requiring TDD scaffolding.
...
**Spec coverage** — every design point from `ONTOLOGY.md` has a task: chronological hierarchy (Tasks 1, 3, 4), build-time tier validation (Task 2), single-parent convention (Task 4's data + Task 2's validator), tier-scoped geography emptiness (Task 3's test + Task 6's documented reasoning), naming/`defines` relation convention (documented in `ONTOLOGY.md`; no code required until something actually needs it — noted, not silently dropped), retiring the competitive visibility algorithm (Task 8), scope-local ranking via `top_entities` (Task 7). Not covered here, and explicitly out of scope: the Holocene/geological display layer (UI-only, future plan). The fine-grained historical-region field, once flagged as an open gap in `ONTOLOGY.md`, was closed by Task 9's `Geography.historical_regions`/`primary_historical_region` (a starter, growable lookup table, not exhaustive, but no longer open).
