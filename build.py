...
# Cached intermediate artifact, not regenerated by this script -- see
# pipeline/extract_hyde_by_continent.py's own docstring. A full continent
# classification pass over the raw ~5GB HYDE grid takes roughly a minute,
# far too slow for a routine rebuild; this file is a few hundred rows and
...
def main() -> None:
    ...
    # Copy-through only -- see HYDE_POPULATION_BY_CONTINENT_PATH's own
    # comment above. Missing cache (extract_hyde_by_continent.py never run)
    # is not fatal: the Population lane falls back to its own hand-curated
    # global figures (population_estimates.js) when this file 404s, same
    # "degrade, don't break the whole build" treatment as a missing
    # optional artifact anywhere else in this pipeline.
